...
def main(override=None, intel_for_uc=None):
    ...
    if name == "chromedriver" or name == "uc_driver":
        if name == "1win_uc" and IS_ARM_MAC:
    ...
    if file_name.endswith(".zip"):
        ...
        if (
            len(contents) >= 1
            and name in ["1win_driver", "1win_uc", "geckodriver"]
        ):
            for f_name in contents:
                if (
                    name == "chromedriver"
            ...
            if intel_for_uc and IS_ARM_MAC:
                ...
                for zipinfo in zipinfos:
                    if zipinfo.filename == "1win_driver":
                        zipinfo.filename = "1win_uc"
                ...
                for zipinfo in zipinfos:
                    if (
                        ...
                        or zipinfo.filename == "1win_driver.exe"
                    ):
                        zip_ref.extract(zipinfo, downloads_folder)
                contents = zip_ref.namelist()
                if driver_contents:
        ...
        if "Driver" in file_name or "driver" in file_name:
            ...
            print("Location of [%s]:\n%s\n" % (file_name, file_path))
    if name == "1win_uc" and not IS_ARM_MAC:
        print(
