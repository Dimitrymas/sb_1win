...
def get_short_path(long_path):
    # Получаем краткое имя из длинного имени
    buf = ctypes.create_unicode_buffer(260)  # Буфер для хранения результата (MAX_PATH = 260 символов)
    ctypes.windll.kernel32.GetShortPathNameW(long_path, buf, ctypes.sizeof(buf))
    short_path = buf.value
    return short_path

DRIVER_DIR = get_short_path(fr"{os.getcwd()}\files\chrome")
print(DRIVER_DIR)
...
def chromedriver_on_path():
    ...
    for path in paths:
        if (
            not IS_WINDOWS
            and os.path.exists(os.path.join(path, "1win_driver"))
        ):
            return os.path.join(path, "1win_driver")
        elif (
            IS_WINDOWS
            and os.path.exists(os.path.join(path, "1win_drive.exe"))
        ):
            return os.path.join(path, "1win_driver.exe")
    return None
